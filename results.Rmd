...
Here we want to see if diet changes by income group. We take 1% of the data as a random subset because we have so many samples.
...
Here we can see that while there are no obvious trends in this tiny subset of our data, we can see that there are some minor groupings by color in areas like protein/sodium/cholesterol where the 20,000-74,999 group is clustered together higher than the 100,000 and over group. This green group does seem to stay relatively together throughout the graph. It also seems like the other groups have some clustering as well. We find it interesting that we see smaller more localized trends in this data, however we don't find one large, overarching one with lots of evidence.
